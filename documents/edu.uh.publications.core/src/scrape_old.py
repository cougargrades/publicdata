...
with open('../core_curriculum_old.csv', 'w', newline='') as export:
  with open('../master.csv', 'r') as masterFile:
    ...
    for catoid in records.keys():
      res2 = requests.get(f'https://publications.uh.edu/content.php?catoid={catoid}&navoid={id}', verify=False)
      if res2.status_code == 200:
        ...
        for coreCode in records[catoid].keys():
          ...
          res = requests.get(f'https://publications.uh.edu/ajax/preview_page.php?catoid={catoid}&id={id}&show', verify=False)
